...
{
        PoseGraph::PoseGraph(const std::map<Timestamp,PoseWithCovariance::ConstShPtr>& poses):_poses(poses){}
        PoseGraph::PoseGraph(const std::map<Timestamp,SE3d>& poses)
        {
        ...
        PoseWithCovariance::ConstShPtr PoseGraph::poseAt(Timestamp t, bool interpolate) const
        {
                auto it = _poses.find(t);
                if(it == _poses.end())
                {
                        return interpolate ? interpolateAt(t) : throw std::runtime_error("No pose at: "+ std::to_string(t));
                }else{
                        return it->second;
                }

        }
        PoseWithCovariance::ConstShPtr PoseGraph::poseBetween(Timestamp t0,Timestamp t1, bool interpolate) const
        ...
        PoseWithCovariance::ConstShPtr PoseGraph::interpolateAt(Timestamp t) const
        {
        ...
        void PoseGraph::append(vision::Timestamp t, PoseWithCovariance::ConstShPtr pose)
        {
                _poses[t] = pose;
        }
