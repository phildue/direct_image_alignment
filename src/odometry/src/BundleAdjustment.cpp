...
namespace pd::vslam::mapping{
        ...
        void BundleAdjustment::setObservation(std::uint64_t pointId, std::uint64_t frameId, const Vec2d& observation)
        {
        ...
        SE3d BundleAdjustment::getPose(std::uint64_t frameId) const
        {
                ...
                if (it != _poses.end() ){ return it->second;}
                else{ throw pd::Exception("Did not find corresponding pose.");}
        }
        Vec3d BundleAdjustment::getPoint(std::uint64_t pointId) const
        {
                auto it = _points.find(pointId);
                if (it != _points.end() ){ return it->second;}
                else{ throw pd::Exception("Did not find corresponding point.");}
