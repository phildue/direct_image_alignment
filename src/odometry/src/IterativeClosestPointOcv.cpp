...
namespace pd::vslam{
        ...
        PoseWithCovariance::UnPtr IterativeClosestPointOcv::align(FrameRgbd::ConstShPtr from, FrameRgbd::ConstShPtr to) const
        {
                ...
                cv::eigen2cv(algorithm::computeRelativeTransform(from->pose().pose(),to->pose().pose()).matrix(),guess);
                
                LOG_ODOM(DEBUG) << "K=" << camMat;
                cv::rgbd::ICPOdometry estimator(camMat,cv::rgbd::Odometry::DEFAULT_MIN_DEPTH(), cv::rgbd::Odometry::DEFAULT_MAX_DEPTH(),
                  cv::rgbd::Odometry::DEFAULT_MAX_DEPTH_DIFF(), cv::rgbd::Odometry::DEFAULT_MAX_POINTS_PART(),std::vector<int>({30,30,30}));
                ...
                if(success)
                {
                        ...
                        return std::make_unique<PoseWithCovariance>( SE3d(Rt) * from->pose().pose(), MatXd::Identity(6,6) );
                }else{
