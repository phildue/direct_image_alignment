...
TEST(WarpSE3Test,Jacobian)
{
        ...
        auto frame = std::make_shared<FrameRgbd>(img,depth,cam,1,0,PoseWithCovariance(pose,Eigen::MatrixXd::Identity(6,6)));
        auto w = std::make_shared<WarpSE3>(pose,frame->pcl(),frame->width(),cam,cam);
        const int u = 240;
        const int v = 300;
        std::vector<double> A_buf(6);
        double* A_ptr = &A_buf[0];
        calcRgbdEquationCoeffs(A_ptr,frame->dIx()(v,u),frame->dIy()(v,u),frame->p3d(v,u),cam->fx(),cam->fy());
        Eigen::Map<Eigen::Vector6d> jacobianOpencv(A_buf.data(),A_buf.size());
        Eigen::Vector6d jacobian = w->J(u,v).row(0) * (double)frame->dIx()(v,u) + w->J(u,v).row(1) * (double)frame->dIy()(v,u);

        std::cout << "OpenCV: " << jacobianOpencv.transpose() << std::endl;
        std::cout << "Ours: " << jacobian.transpose() << std::endl;
        std::cout << "Ours: " << ourJ(frame->dIx()(v,u),frame->dIy()(v,u),frame->p3d(v,u),cam->fx(),cam->fy()).transpose() << std::endl;
